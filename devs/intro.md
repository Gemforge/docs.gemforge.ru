...
- Миграции - это механизм обновления и отслеживания состоянии базы данных. По мути миграции это такой `git` для базы данных.

> Интересно, что в ранних версиях Prisma (до 2.0), там действидельно был GraphQL и стратегия была направленна на то, чтобы использовать одну спецификацию для доступа к данным и API. Но со временем стало ясно, что Data Access и API сильно разные задачи.
...
Я описывал в статье в нашем блока, про то почему стоит использовать `GraphQL`

В общем мы делаем практически весь API через единый граф данных.

> Стоит отметить, что DAL и API не сильзо сцеплены, это позволяет использовать их независимо, тестировать отдельно. Также API может использовать не только DAL prisma, но и перенаправлять запросы на другие сервисы или загружать данные из файлов.
